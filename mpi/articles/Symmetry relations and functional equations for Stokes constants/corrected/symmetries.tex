...
One can argue that the F-matrix method, developed by Fr\"omans\cite{frbook}, 
operates with the same thing as the effective Stokes constant defined above; and he will be right. 
Nevertheless, there is an important difference between the effective Stokes constant and the Stokes constant
used in the F-matrix method. We assume effective Stokes constant to be analytical function with respect to 
the problem`s parameters $\lmbd$.
...
As it follows from the definition of the effective Stokes constant, it always can be expressed
in terms of traditional Stokes constants and corresponding phase integrals. However, it
stays meaningful even then the traditional Stokes constants do not.
Let`s look at \fref{fig:effsd_2}. Comparing \fref{fig:effsd_2}(a) with \fref{fig:effsd_2}(b),
one can notice that the effective Stokes constant $s$ is actually a combination of
two traditional constants $s_1$ and $s_2$. To calculate, for example, $s_1$, 
we have to continue some solution of the corresponding equation analytically along 
the path $\gamma_2$ shown in \fref{fig:effsd_2}(c) and compare the solution`s phase-integral asymptotics 
on the neighbouring anti-Stokes lines; such a procedure was implemented, for example, in \cite{ours}. 
...
far away from all the singularities and $s$ from \fref{fig:effsd_2}(b) remains computable and meaningful. 

We assume effective Stokes constant to be analytical function with respect to the problem`s parameters $\lmbd$.
Certainly, assuming this 
