...
the properties of the integral defined in \cite{frpaper}, in a given Stokes domain, 
these operators forms a multiplicative group too:
\begin{eqnarray}
\W[c,b]\W[b,a] = \W[c,a].
\end{eqnarray}
And, completely analogous to the situation discussed above, every set of sequential changes of the basepoint
can be described by the only one reconnection operator.
...
\end{eqnarray}
But we always can change the order of $\S$ and $\W$ operators because $\S[s']\W[b,a]=\W[b,a]\S[s'']$. Hence,
every such set of operators can always be replaced by only one combination $\S \W$ or, 
if someone prefers different ordering, $\W \S$. Consequently, every Stokes domain can be described by 
only one effective Stokes constant despite the number of Stokes lines it contains. The difference between 
...
1. A traditional Stokes diagram; dots mark the zeros of the phase integrand
...
and can be plotted differently depending on the chosen path of analytical continuation; that is why
...
will be used in the given Stokes domain. Also it can be useful to indicate values of the phase integrals 
instead of plotting multiple branch cuts.
...
some solution of the corresponding equation analytically along the path shown in  \fref{fig:effsd_2}.3. Nevertheless,
...
far away from all the singularities and $s$ from \fref{fig:effsd_2} remains measurable and meaningful
...
%$\{\g=\rme^{2 \rmi \pi/3}\unity,\f=\h=\unity\}$ and $\{\h=\unity,\f=\g=c.c.\}$, where $\unity$ 
%is a unity operator and $c.c.$ stands 
%for complex conjugate. The main difference between these two symmetries is that the second 
%one changes a direction of a complex variable`s phase increase and the first one does not. 
%We will refer to the symmetries of the first kind as rotation symmetries and of the second 
...
associated with some other effective Stokes constant $s'(\h\lmbd)$ (\fref{fig:rst}.3). Now we can see that
...
in the phase integrals in \eref{eq:func}, let`s parametrize $\h$ as $\h_{cont}(\mu)=\rme^{\rmi\pi\mu}\unity$ 
...
It is interesting to investigate \eref{eq:s3/2} from the perspective of the approximation
...
fails and $s_{3/2}(\delta)$ changes its limiting value. As it can be inferred from \fref{fig:webrs},
...
Relation \eref{eq:s3/2} is in a perfect agreement with the reasoning.
